...
public class OmsConditionServiceImpl implements OmsConditionService {
    ...
    private void remindCheckApply(ConditionReplaceVO conditionReplaceVO, String type){
        ...
        if (omsConditions != null && omsConditions.size() > 0){
            //查询关键词
            QueryWrapper<OmsReplaceKeywords> keywords = new QueryWrapper<>();
            keywords.eq("TYPE", type)
                    .eq("USE_TYPE", Constants.OMS_KEYWORDS_USERTYPE[1]);
            List<OmsReplaceKeywords> omsReplaceKeywords = omsReplaceKeywordsMapper.selectList(keywords);
            //检验条件
            for (OmsCondition omsCondition : omsConditions) {
                //替换关键词，执行sql
                ...
                if ("1".equals(fit)){
                    ...
                    }else if (Constants.OMS_CONDITION_REMIND_PERSONTYPE[1].equals(omsCondition.getRemindPersonType())){
                        //组团单位 TODO
                    }else if (Constants.OMS_CONDITION_REMIND_PERSONTYPE[2].equals(omsCondition.getRemindPersonType())){
                        //审批单位 TODO
                    }else if (Constants.OMS_CONDITION_REMIND_PERSONTYPE[3].equals(omsCondition.getRemindPersonType())){
                        //干部监督处 TODO
                    }
