...
public class ReturnCerInfo {

    @NotBlank(message = "备案表id不能为空")
    @ApiModelProperty(value="备案表id",required = true)
    ...
    @NotNull(message = "保管单位不能为空")
    @ApiModelProperty(value="保管单位(0:干部监督处,1:省委统战部(台办))",required = true)
    private String surelyUnit;

    @NotNull(message = "保管方式不能为空")
    @ApiModelProperty(value="保管方式(0:证照机,1:柜台)",required = true)
    private String surelyWay;

    @NotNull(message = "柜台编号不能为空")
    @ApiModelProperty(value="柜台编号",required = true)
