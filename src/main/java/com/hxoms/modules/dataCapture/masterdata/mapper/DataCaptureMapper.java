...
public interface DataCaptureMapper {
     ...
     * PersonOrgOrder 表
     */
    List<Map> getMasterPersonOrgOrder(@Param("offset") int offset, @Param("rows") int rows);

    Integer getMasterPersonOrgOrderCount();

    void updateMasterPersonOrgOrder(List<Map> updateList);

    void insertMasterPersonOrgOrder(List<Map> insertList);
}
